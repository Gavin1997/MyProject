<template>
  <div>
    <section class="bg-gray p-0 m-0" v-cloak>
                ...
                <ul class="list-unstyle d-flex">
                  <li class="">
                    ...
                    <div class="input_count ml-3">
                      <button :class="{disabled:isover}" @click.prevent="modifed(-1,$event)">-</button>
                      <span>{{aduts_count}}</span>
                      <button :class="{disabled:isover_add}" @click.prevent="modifed(1,$event)">+</button>
                    </div>
                  </li>
                  <li class="ml-5">
                    <h4>儿童
                    ...
                    <div class="input_count ml-3">
                      <button :class="{disabled:isover_child}" @click.prevent="child_modifed(-1,$event)">-</button>
                      <span>{{child_count}}</span>
                      <button :class="{disabled:isover_add}" @click.prevent="child_modifed(1,$event)">+</button>
                    </div>
                  </li>
                </ul>
              </div>
      ...
      <!-- 下方大图详情 -->
      <section-Details-bottom :lg_pic="lg_pic" :departure="departure" :iffixed="iffixed"></section-Details-bottom>
    </section>
  </div>
</template>
<script>
  ...
  export default {
    data() {
      return {
        ...
        lg_pic: "",//大图
        departure: "",//出发地
        aduts_count: 1, //成人购买数量
        child_count: 1,//儿童各买数量
        tid: this.$route.params.tid,//当前产品的tid
        changedActive: 0,//当前内容类型是否高亮显示
        isover:false,//添加加减的按钮是否满足条件
        isover_add:false,//加按钮是否隐藏
        isover_child:false//儿童价格减按钮是否满足条件
      }
    },
    props:["iffixed"],
    methods: {
      ...
      child_modifed(i, e) {
        ...
        if(this.child_count>=0){
          this.isover_child = false
        }else{
          this.isover_child = true;
          this.child_count = 0;
        }
      },
